...
void MeshPartitioner<SC,LO,GO,NO>::readAndPartitionMesh( int meshNumber ){
    ...
    meshUnstr->mapRepeated_.reset( new Map_Tpetra<LO,GO,NO>(OTGO::invalid(), pointsRepGlobMapping, 0, this->comm_) );
    ...
    meshUnstr->elementsC_.reset(new Elements ( FEType, dim ) );
    {
        ...
        meshUnstr->elementMap_.reset(new Map_Tpetra<LO,GO,NO>( (GO) -1, elementsGlobalMapping, 0, this->comm_) );
    ...
    if (buildEdges) {
        ...
        meshUnstr->edgeMap_.reset(new Map_Tpetra<LO,GO,NO>( (GO) -1, edgesGlobalMapping, 0, this->comm_) );
    }
