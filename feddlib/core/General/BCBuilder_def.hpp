...
void BCBuilder<SC,LO,GO,NO>::determineVelocityForFlowrate(LO i, double time) const{
    ...
    MultiVectorPtr_Type parabolic_unique =  Teuchos::rcp_const_cast<MultiVector_Type> ( vecExternalSol_[i]); // e.g. Laplace soution on inlet
    ...
    // double area = 0.;
    // feFactory->assemblyArea(domain->getDimension(), area, vecFlag_[i]);
    // std::cout << " Area inlet " << area << std::endl;
