...
class AssembleFE_NonLinElas : public AssembleFE<SC,LO,GO,NO> {
  public:
   ...
   private:
	...
	std::vector<double> ul_;  // displacement vector
	std::vector<double> ul0_; // unused
	std::vector<double> xl_;  // nodal coordinates
	std::vector<double> s_;   // stiffness matrix
	std::vector<double> p_;   // residual vector
