...
UN Helper::determineDegree2(UN dim, std::string FEType1, std::string FEType2,
                            VarType type1, VarType type2, UN extraDeg) {
...
int Helper::getDPhi(vec3D_dbl_ptr_Type &DPhi, vec_dbl_ptr_Type &weightsDPhi,
...
void Helper::gradPhi(int dim, int intFE, int i, vec_dbl_Type &p,
...
void Helper::phi(int dim, int intFE, int i, vec_dbl_Type &p, double *value) {
...
int Helper::getPhi(vec2D_dbl_ptr_Type &Phi, vec_dbl_ptr_Type &weightsPhi,
                   int dim, std::string FEType, int Degree,
...
\brief Returns neccesary quadrature Values. Is distinguishes between needing
Element or Surface information. !! Input can be improved with just delivering
the coordinates of the surface nodes to determine the quad points

@param[in] dim Dimension for which the quadrature points are needed.
@param[in] FEType Finite element type for which the quadrature points are
needed.
...
@param[in] points The repeated(!) points of current problem to identify the
surface node ids.

@param[out] QuadPts Quadrature points
@param[out] QuadW Quadrature weights

\brief Keep in mind that elementwise quadPoints are defined on reference element
whereas surface quadPoints at hand are defined on the input surface, which is
typically not the reference Element.

*/

vec2D_dbl_Type Helper::getQuadratureValuesOnSurface(int dim, std::string FEType,
                                                    vec_dbl_Type &QuadW,
    ...
    if (dim == 2) {
        ...
        if (FEType == "P1") {
            // In my case: As nabla phi is a constant function, quad points
            // don't really matter in that case ...
            QuadPts[0][0] = 1 / 3.;
            QuadPts[0][1] = 1 / 3.;
            QuadPts[0][2] = 1 / 3.;
...
void Helper::getQuadratureValues(int dim, int Degree,
                                 vec2D_dbl_ptr_Type &QuadPts,
