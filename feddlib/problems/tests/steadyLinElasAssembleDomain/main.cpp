...

    {
       ...
       // ########################
        ...
        int itsAssFE;
        {
            ...
            LinElasAssembleDomain.addBoundaries(bcFactory); // Dem Problem RW hinzufuegen

            if (dim==2)
                LinElasAssembleDomain.addRhsFunction( rhs2D );
            else if(dim==3)
                LinElasAssembleDomain.addRhsFunction( rhsYZ);

            double force = parameterListAll->sublist("Parameter").get("Volume force",0.);

            LinElasAssembleDomain.addParemeterRhs( force );
            // ######################
            // Matrix assemblieren, RW setzen und System loesen
            // ######################
            LinElasAssembleDomain.initializeProblem();
            LinElasAssembleDomain.assemble();                
            LinElasAssembleDomain.setBoundaries(); // In der Klasse Problem
            itsAssFE = LinElasAssembleDomain.solve();
        }
        
		if(comm->getRank() ==0){
			cout << " ############################################### " << endl;
			cout << " Linear Iterations FEDDLib Old Assembly : " << its << endl; 
			cout << " Linear Iterations FEDDLib Domain Assembly  : " << itsAssFE << endl;
			cout << " ############################################### " << endl;
		...
		MultiVectorConstPtr_Type valuesSolidConst2 = LinElasAssembleDomain.getSolution()->getBlock(0);
		...
		LinElasAssembleDomain.getSolution()->norm2(norm);
		res = norm[0];
		if(comm->getRank() ==0)
       ...
       TEUCHOS_TEST_FOR_EXCEPTION( infNormError > 1e-12 , std::logic_error, "Inf Norm of Error between calculated solutions is too great. Exceeded 1e-12 ");
