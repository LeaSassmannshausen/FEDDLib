...

  {
...

    {
      ...
      std::vector<double> parameter_vec(
          1, parameterListProblem->sublist("Parameter").get("MaxVelocity", 1.));
      ...
      if (dim == 2) {
        bcFactory->addBC(zeroDirichlet2D, 1, 0, domainVelocity, "Dirichlet",
                         dim);
        bcFactory->addBC(inflowParabolic2D, 2, 0, domainVelocity, "Dirichlet",
                         dim, parameter_vec);
        //                       bcFactory->addBC(dummyFunc, 3, 0,
        //                       domainVelocity, "Neumann", dim);
        //                        bcFactory->addBC(dummyFunc, 666, 1,
        //                        domainPressure, "Neumann", 1);
        // bcFactory->addBC(zeroDirichlet2D, 3, 0, domainVelocity, "Dirichlet",
        // dim);
        bcFactory->addBC(zeroDirichlet2D, 4, 0, domainVelocity, "Dirichlet",
                         dim);
        bcFactory->addBC(zeroDirichlet2D, 5, 0, domainVelocity, "Dirichlet",
                         dim);
      } else if (dim == 3) {
        bcFactory->addBC(zeroDirichlet3D, 1, 0, domainVelocity, "Dirichlet",
                         dim);
        bcFactory->addBC(inflowParabolic3D, 2, 0, domainVelocity, "Dirichlet",
                         dim, parameter_vec);
        //                        bcFactory->addBC(dummyFunc, 3, 0,
        //                        domainVelocity, "Neumann", dim);
        //                        bcFactory->addBC(dummyFunc, 666, 1,
        //                        domainPressure, "Neumann", 1);
        bcFactory->addBC(zeroDirichlet3D, 2, 0, domainVelocity, "Dirichlet",
                         dim);
      }
      // Flag Check
      ...
      /*       	NavierStokes<SC,LO,GO,NO> navierStokes( domainVelocity,
       * discVelocity, domainPressure, discPressure, parameterListAll ); */
      /**/
      /*         { */
      ...
      /* res = norm[0]; */
      /* if (comm->getRank() == 0) */
      /*   cout << " 2 rel. Norm of solution navier stokes " << twoNormError /
       * res */
      /*        << endl; */
      ...
      /* res = norm[0]; */
      /* if (comm->getRank() == 0) */
      /*   cout << " 2 rel. Norm of solutions navier stokes assemFE " */
      /*        << twoNormError / res << endl; */
      /**/
      /* MatrixPtr_Type Sum2 = Teuchos::rcp( */
      /*     new Matrix_Type(domainVelocity->getMapVecFieldUnique(), */
      /*                     domainVelocity->getDimension() * */
      /*                         domainVelocity->getApproxEntriesPerRow())); */
      ...
      /* res = 0.; */
      /* for (UN i = 0; i < domainVelocity->getMapUnique()->getMaxLocalIndex() +
       * 1; */
      /*      i++) { */
      /*   for (int d = 0; d < dim; d++) { */
      /*     GO row = */
      /*         dim * domainVelocity->getMapUnique()->getGlobalElement(i) + d;
       */
       ...
       */
      /**/
      /* MatrixPtr_Type Sum1 = Teuchos::rcp( */
      /*     new Matrix_Type(domainPressure->getMapUnique(), */
      /*                     domainVelocity->getDimension() * */
      /*                         domainVelocity->getApproxEntriesPerRow())); */
      ...
      /**/
      /* res = 0.; */
      /* for (UN i = 0; i < domainPressure->getMapUnique()->getMaxLocalIndex() +
       * 1; */
      /*      i++) { */
      ...
      /**/
      /* UN dofsPerNode = dim; */
      /* exParaVelocity->addVariable(exportSolutionV, "u", "Vector",
       * dofsPerNode, */
      /*                             dom->getMapUnique()); */
      /* exParaVelocity->addVariable(exportSolutionVAssFE, "uAssFE", "Vector",
       */
      /*                             dofsPerNode, dom->getMapUnique()); */
      /* exParaVelocity->addVariable(errorValuesAbs, "u-uAssFE", "Vector", */
      /* dom = domainPressure; */
      /* exParaPressure->setup("pressure", dom->getMesh(), dom->getFEType()); */
      /**/
      /* exParaPressure->addVariable(exportSolutionP, "p", "Scalar", 1, */
      /* exParaPressure->addVariable(exportSolutionPAssFE, "pAssFE", "Scalar",
       * 1, */
      /*                             dom->getMapUnique()); */
      /**/
      ...
      // TEUCHOS_TEST_FOR_EXCEPTION( infNormError > 1e-11 , std::logic_error,
      // "Inf Norm of Error between calculated solutions is too great. Exceeded
      // 1e-11. ");
    }
  }
