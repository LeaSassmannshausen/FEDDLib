...
#include "feddlib/problems/specific/AssembleDomain.hpp"
...
//                bcFactory->addBC(dummyFunc, 666, 1, domainPressure, "Neumann", 1);
            ...
            {
				MAIN_TIMER_START(Stokes," Assemble System and solve");
       ...
       // /*
        ...
        cout << "###############################################################" <<endl;
            ...
            domainArray[0] = domainVelocity;
            domainArray[1] = domainPressure;
            AssembleDomain<SC,LO,GO,NO> stokesAssembleDomain(domainArray,parameterListAll,"Stokes");
            {
                MAIN_TIMER_START(StokesAssembleDomain," AssembleDomain: Assemble System and solve");

                stokesAssembleDomain.addBoundaries(bcFactory);
                
                stokesAssembleDomain.initializeProblem();
                stokesAssembleDomain.assemble();
                stokesAssembleDomain.setBoundaries();
                stokesAssembleDomain.solve();

	            MAIN_TIMER_STOP(StokesAssembleDomain);	
                comm->barrier();
            }
...
///*
            Teuchos::RCP<const MultiVector<SC,LO,GO,NO> > exportSolutionVAssFE = stokesAssembleDomain.getSolution()->getBlock(0);
            Teuchos::RCP<const MultiVector<SC,LO,GO,NO> > exportSolutionPAssFE = stokesAssembleDomain.getSolution()->getBlock(1);
			...
			stokes.getSolution()->norm2(norm);
			res = norm[0];
			if(comm->getRank() ==0)
				cout << " 2 rel. Norm of solution navier stokes " << twoNormError/res << endl;

			stokesAssembleDomain.getSolution()->norm2(norm);
			res = norm[0];
			if(comm->getRank() ==0)
			...
			stokes.getSystem()->getBlock(0,0)->addMatrix(1, Sum2, 1);
			stokesAssembleDomain.getSystem()->getBlock(0,0)->addMatrix(-1, Sum2, 1);
			...
			stokes.getSystem()->getBlock(1,0)->addMatrix(1, Sum1, 1);
			stokesAssembleDomain.getSystem()->getBlock(1,0)->addMatrix(-1, Sum1, 1);

            stokes.getSystem()->getBlock(1,0)->print();

            stokesAssembleDomain.getSystem()->getBlock(1,0)->print();
