...
void rhsHeartBeatArtery(double* x, double* res, double* parameters){
    ...
    double t_min = heartBeatStart+0.5-0.02; //FlowConditions::t_start_unsteady;
...
int main(int argc, char *argv[])
{
    ...
    TimeMonitor::setStackedTimer(stackedTimer);
    {
        ...
        {
            std::cout << "\t  ###############################################" <<endl;
            std::cout << "\t  ############ sci_Test.main:: INFOs ############" <<endl;
        }
        if (verbose)
        {
            if (!meshType.compare("structured")) {
                std::cout << "\t ---- Building structured mesh based on number of processors and H/h ----" <<endl;
                ...
                std::cout << " ---- Reading unstructured mesh based on inputfile: 'Mesh 1 Name' ----" <<endl;
            }
        }
        ...
        if(dim == 3 && bcType=="Cube")
        {
            ...
            bcFactoryStructure->addBC(zeroDirichlet3D, 9, 0, domainStructure, "Dirichlet_X_Z", dim);
             ...
             if(verbose){
                std::cout << " \t Boundary Condition type Artery. This boundary condition is for longer and full artery like geometry (or tube) with length c. " << std::endl;
        ...
        if (verbose)
        {
            ...
            std::cout << "\t  ###############################################" <<endl;
            std::cout << "\t  start solving problem ..." <<endl;
        }
