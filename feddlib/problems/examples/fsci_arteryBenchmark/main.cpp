...
/*! Test case for the curved artery gemeotry proposed in 
    ...
    -> The test case is similar to fsi_arteryBenchmark, but now using the FSCI class for adding a chemical diffusion equation
*/
...
void flowrate3D(double* x, double* res, double t, const double* parameters)
{
    ...
    else if(t > heartBeatStart)
    {
        ...
        res[0] =  parameters[2] + (parameters[2]+1) * Q  - 0.13 ; // 1.6563
...
int main(int argc, char *argv[])
{
    ...
    string xmlSolverFileFSCI = "parametersSolverFSCI.xml"; // GI
    myCLP.setOption("solverfileFSCI",&xmlSolverFileFSCI,".xml file with Inputparameters.");
...

    {
        ...
        ParameterListPtr_Type parameterListSolverFSCI = Teuchos::getParametersFromXmlFile(xmlSolverFileFSCI);
        ...
        parameterListAll->setParameters(*parameterListSolverFSCI);
        ...
        parameterListChemAll->setParameters(*parameterListSolverFSCI);
        ...
        // #####################
        {
            ...
            {
                cout << "###############################################" <<endl;
                cout << "####### Starting FSCI Benchmark  ... ##########" <<endl;
                cout << "###############################################" <<endl;
            }
           ...
           Teuchos::RCP<SmallMatrix<int>> defTS;
            ...
            Teuchos::RCP<BCBuilder<SC,LO,GO,NO> > bcFactorySCI( new BCBuilder<SC,LO,GO,NO>( ) );
            {
                ...
                bcFactorySCI->addBC(zeroDirichlet3D, 4, 0, domainStructure, "Dirichlet_Z", dim);           
                bcFactorySCI->addBC(zeroDirichlet3D, 5, 0, domainStructure, "Dirichlet_X", dim); 
            ...
            if (!fsci.problemStructure_.is_null())
                fsci.problemStructure_->addRhsFunction( rhsDummy );
            else
                fsci.problemStructureNonLin_->addRhsFunction( rhsDummy );
