...
void sDummyFunc(double *x, double *res, double t, double *parameter) { return; }

double OneFunction(double *x, int *parameter) { return 1.0; }

namespace FEDD {

template <class SC, class LO, class GO, class NO>
NavierStokes<SC, LO, GO, NO>::NavierStokes(
    ...
    : NonLinearProblem<SC, LO, GO, NO>(parameterList,
                                       domainVelocity->getComm()),
      A_(), pressureIDsLoc(new vec_int_Type(2)), u_rep_() {

  this->nonLinearTolerance_ =
      this->parameterList_->sublist("Parameter").get("relNonLinTol", 1.0e-6);
  this->initNOXParameters();

  this->addVariable(domainVelocity, FETypeVelocity, "u",
                    domainVelocity->getDimension());
  ...
  if (parameterList->sublist("Parameter")
          .get("Calculate Coefficients", false)) {
...
void NavierStokes<SC, LO, GO, NO>::assembleConstantMatrices() const {
  ...
  if (this->verbose_)
    std::cout << "-- Assembly constant matrices Navier-Stokes ... "
              << std::flush;

  double viscosity =
      this->parameterList_->sublist("Parameter").get("Viscosity", 1.);
  double density =
  ...
  A_.reset(new Matrix_Type(this->getDomain(0)->getMapVecFieldUnique(),
                           this->getDomain(0)->getDimension() *
                               this->getDomain(0)->getApproxEntriesPerRow()));

  if (this->parameterList_->sublist("Parameter")
          .get("Symmetric gradient", false))
    this->feFactory_->assemblyStress(this->dim_, this->domain_FEType_vec_.at(0),
                                     A_, OneFunction, dummy, true);
  else
    this->feFactory_->assemblyLaplaceVecField(
        this->dim_, this->domain_FEType_vec_.at(0), 2, A_, true);
  ...
  A_->fillComplete(this->getDomain(0)->getMapVecFieldUnique(),
  ...
  if (!this->parameterList_->sublist("General")
           .get("Preconditioner Method", "Monolithic")
           .compare("Teko")) {
    if (!this->parameterList_->sublist("General").get("Assemble Velocity Mass",
                                                      false)) {
      MatrixPtr_Type Mvelocity(
          new Matrix_Type(this->getDomain(0)->getMapVecFieldUnique(),
                          this->getDomain(0)->getApproxEntriesPerRow()));
      //
      this->feFactory_->assemblyMass(this->dim_, this->domain_FEType_vec_.at(0),
                                     "Vector", Mvelocity, true);
      //
      this->getPreconditionerConst()->setVelocityMassMatrix(Mvelocity);
      if (this->verbose_)
        std::cout << "\nVelocity mass matrix for LSC block preconditioner is "
                     "assembled."
                  << std::endl;
    } else {
      if (this->verbose_)
        std::cout << "\nVelocity mass matrix for LSC block preconditioner not "
                     "assembled."
                  << std::endl;
    }
  }
#endif
  string precType = this->parameterList_->sublist("General").get(
      "Preconditioner Method", "Monolithic");
  if (precType == "Diagonal" || precType == "Triangular") {
    MatrixPtr_Type Mpressure(
        new Matrix_Type(this->getDomain(1)->getMapUnique(),
                        this->getDomain(1)->getApproxEntriesPerRow()));

    this->feFactory_->assemblyMass(this->dim_, this->domain_FEType_vec_.at(1),
                                   "Scalar", Mpressure, true);
    SC kinVisco =
        this->parameterList_->sublist("Parameter").get("Viscosity", 1.);
...
void NavierStokes<SC, LO, GO, NO>::assembleDivAndStab() const {
  ...
  MatrixPtr_Type B(new Matrix_Type(
      pressureMap, this->getDomain(0)->getDimension() *
                       this->getDomain(0)->getApproxEntriesPerRow()));

  MatrixPtr_Type C;

  this->feFactory_->assemblyDivAndDivTFast(
      this->dim_, this->getFEType(0), this->getFEType(1), 2, B, BT,
      this->getDomain(0)->getMapVecFieldUnique(), pressureMap, true);
  ...
  if (!this->getFEType(0).compare("P1")) {
    C.reset(new Matrix_Type(this->getDomain(1)->getMapUnique(),
                            this->getDomain(1)->getApproxEntriesPerRow()));
...
template <class SC, class LO, class GO, class NO>
void NavierStokes<SC, LO, GO, NO>::reAssemble(MatrixPtr_Type &massmatrix,
                                              std::string type) const {}

template <class SC, class LO, class GO, class NO>
void NavierStokes<SC, LO, GO, NO>::reAssembleFSI(std::string type,
                                                 MultiVectorPtr_Type u_minus_w,
                                                 MatrixPtr_Type P) const {

  if (this->verbose_)
    std::cout << "-- Reassembly Navier-Stokes (" << type << ") for FSI ... "
              << std::flush;
  ...
  if (type == "FixedPoint") {
    ...
    this->feFactory_->assemblyAdvectionVecField(
        this->dim_, this->domain_FEType_vec_.at(0), N, u_minus_w, true);
...
template <class SC, class LO, class GO, class NO>
void NavierStokes<SC, LO, GO, NO>::reAssembleExtrapolation(
    BlockMultiVectorPtrArray_Type previousSolutions) {

  if (this->verbose_)
    std::cout << "-- Reassembly Navier-Stokes (Extrapolation) ... "
              << std::flush;
  ...
  if (previousSolutions.size() >= 2) {
    ...
    MultiVectorPtr_Type extrapolatedVector =
        Teuchos::rcp(new MultiVector_Type(previousSolutions[0]->getBlock(0)));
...
//     MultiVector_ptr_vec_ptr_Type sol_unique_vec(new
//     std::vector<MultiVector_ptr_Type>(0));
//     this->system_->FillSplitVector64(*this->solution_, sol_unique_vec);
//     this->system_->BuildRepeatedVectorBlocks(sol_unique_vec);
//     Vector_ptr_Type u_rep(new
//     Epetra_Vector(*(this->system_->GetRepeatedVec(0))));
//     Teuchos::RCP<Epetra_FECrsMatrix> 	N(new
//     Epetra_FECrsMatrix(Epetra_DataAccess::Copy,*(this->domainPtr_vec_.at(0)->GetMapXDimUnique()),10));
//     u_rep.reset(new Epetra_Vector(*(this->system_->GetRepeatedVec(0))));
//     N.reset(new
//     Epetra_FECrsMatrix(Epetra_DataAccess::Copy,*(this->domainPtr_vec_.at(0)->GetMapXDimUnique()),10));
//     this->feFactory_->AssemblyAdvectionXDim(dimension,
//     this->domain_FEType_vec_.at(0), 7, N, u_rep /* u */, setZeros);
//
//     Teuchos::RCP<Epetra_CrsMatrix>   	AN(new
//     Epetra_CrsMatrix(Epetra_DataAccess::Copy,*(this->domainPtr_vec_.at(0)->GetMapXDimUnique()),10));
//
...
//
//
//     Teuchos::RCP<Epetra_FECrsMatrix> 	B_T (new
//     Epetra_FECrsMatrix(Epetra_DataAccess::Copy,*(this->domainPtr_vec_.at(0)->GetMapXDimUnique()),5));
//     Teuchos::RCP<Epetra_FECrsMatrix> 	B (new
//     Epetra_FECrsMatrix(Epetra_DataAccess::Copy,*(this->domainPtr_vec_.at(1)->GetMapUnique()),5));
//     this->feFactory_->AssemblyDivergence(dimension,
//     this->domain_FEType_vec_.at(0), this->domain_FEType_vec_.at(1), 2, B,
//     B_T, setZeros, this->domainPtr_vec_.at(0)->GetMapXDimUnique(),
//     this->domainPtr_vec_.at(1)->GetMapUnique()); B_T->Scale(-1.);
...
//
//     Teuchos::RCP<BCBuilder> bCFactoryDrag(new
//     BCBuilder(sublist(this->parameterList_,"Parameter")));
//     Teuchos::RCP<BCBuilder> bCFactoryLift(new
//     BCBuilder(sublist(this->parameterList_,"Parameter")));
//
//     bCFactoryDrag->AddBC(sxOne2D, 4, 0, this->domainPtr_vec_.at(0),
//     "Dirichlet", dimension); bCFactoryDrag->AddBC(sDummyFunc, 666, 1,
//     this->domainPtr_vec_.at(1), "Neumann", 1);
//
//     bCFactoryLift->AddBC(syOne2D, 4, 0, this->domainPtr_vec_.at(0),
//     "Dirichlet", dimension); bCFactoryLift->AddBC(sDummyFunc, 666, 1,
//     this->domainPtr_vec_.at(1), "Neumann", 1);
//
//     Teuchos::RCP<Epetra_Vector>  dragVec(new
//     Epetra_Vector(*(*this->solution_)(0))); Teuchos::RCP<Epetra_Vector>
//     liftVec(new Epetra_Vector(*(*this->solution_)(0)));
//     dragVec->PutScalar(0.);
//     liftVec->PutScalar(0.);
//     bCFactoryDrag->SetRHS(this->system_, dragVec);
//     bCFactoryLift->SetRHS(this->system_, liftVec);
//
//     Teuchos::RCP<Epetra_Vector>	mat_sol(new
//     Epetra_Vector(*(*this->solution_)(0)));
...
//     }
//     Teuchos::RCP<Epetra_Vector>     pressureSolutuion( new
//     Epetra_Vector(*((*(sol_unique_vec->at(1)))(0)))); double p1 =
//     numeric_limits<double>::min(); double p2 = numeric_limits<double>::min();
...
// template<class SC,class LO,class GO,class NO>
// typename NavierStokes<SC,LO,GO,NO>::MultiVector_Type
// NavierStokes<SC,LO,GO,NO>::GetExactSolution(double time){ #ifdef
// ASSERTS_WARNINGS
...
// template<class SC,class LO,class GO,class NO>
// void NavierStokes<SC,LO,GO,NO>::set_x0(const Teuchos::ArrayView<const SC>
// &x0_in){ #ifdef TEUCHOS_DEBUG
...
template <class SC, class LO, class GO, class NO>
void NavierStokes<SC, LO, GO, NO>::evalModelImpl(
    const Thyra::ModelEvaluatorBase::InArgs<SC> &inArgs,
    const Thyra::ModelEvaluatorBase::OutArgs<SC> &outArgs) const {
  std::string type = this->parameterList_->sublist("General").get(
      "Preconditioner Method", "Monolithic");
...
template <class SC, class LO, class GO, class NO>
void NavierStokes<SC, LO, GO, NO>::evalModelImplMonolithic(
    const Thyra::ModelEvaluatorBase::InArgs<SC> &inArgs,
    const Thyra::ModelEvaluatorBase::OutArgs<SC> &outArgs) const {
  ...
  if (fill_f || fill_W || fill_W_prec) {
    ...
    if (fill_f) {
      ...
      system_updated = true;
      Teuchos::RCP<Thyra::MultiVectorBase<SC>> f_thyra =
    ...
    if (fill_W) {
      ...
      Teuchos::RCP<TpetraOp_Type> W_tpetra =
          tpetra_extract::getTpetraOperator(W_out);
      Teuchos::RCP<TpetraMatrix_Type> W_tpetraMat =
          Teuchos::rcp_dynamic_cast<TpetraMatrix_Type>(W_tpetra);

      XpetraMatrixConstPtr_Type W_systemXpetra =
          this->getSystem()->getMergedMatrix()->getXpetraMatrix();
    ...
    if (fill_W_prec) {
      ...
      if (!levelCombination.compare("Multiplicative")) {
        ...
        //                ParameterListPtr_Type solverPList =
        //                this->getLinearSolverBuilder()->getNonconstParameterList();
        //
        //                solverPList->sublist("Preconditioner
        //                Types").sublist("FROSch").set("Only apply
        //                coarse",true);
        //
        //                Teuchos::RCP<const Thyra::LinearOpBase<SC> >
        //                thyra_linOp =
        //                this->getPreconditionerConst()->getThyraPrecConst()->getUnspecifiedPrecOp();
        //
        //                f_out->describe(*out,Teuchos::VERB_EXTREME);
        //                vecThyraNonConst->describe(*out,Teuchos::VERB_EXTREME);
        //                Thyra::apply( *thyra_linOp, Thyra::NOTRANS, *f_out,
        //                vecThyraNonConst.ptr() );
        //                solverPList->sublist("Preconditioner
        //                Types").sublist("FROSch").set("Only apply
        //                coarse",false);
      }
...
#ifdef FEDD_HAVE_TEKO
template <class SC, class LO, class GO, class NO>
void NavierStokes<SC, LO, GO, NO>::evalModelImplBlock(
    const Thyra::ModelEvaluatorBase::InArgs<SC> &inArgs,
    const Thyra::ModelEvaluatorBase::OutArgs<SC> &outArgs) const {
  ...
  RCP<Thyra::ProductVectorBase<SC>> vecThyraBlock =
      rcp_dynamic_cast<Thyra::ProductVectorBase<SC>>(vecThyraNonConst);

  this->solution_->getBlockNonConst(0)->fromThyraMultiVector(
      vecThyraBlock->getNonconstVectorBlock(0));
  this->solution_->getBlockNonConst(1)->fromThyraMultiVector(
      vecThyraBlock->getNonconstVectorBlock(1));
  ...
  if (fill_f || fill_W || fill_W_prec) {
    ...
    if (fill_f) {
      ...
      Teko::MultiVector f1;
      f0 =
          this->getResidualVector()->getBlockNonConst(0)->getThyraMultiVector();
      f1 =
          this->getResidualVector()->getBlockNonConst(1)->getThyraMultiVector();

      std::vector<Teko::MultiVector> f_vec;
      f_vec.push_back(f0);
      f_vec.push_back(f1);
    ...
    if (fill_W) {
      ...
      RCP<ThyraBlockOp_Type> W_blocks =
          rcp_dynamic_cast<ThyraBlockOp_Type>(W_out);
      RCP<const ThyraOp_Type> W_block00 = W_blocks->getBlock(0, 0);
      RCP<ThyraOp_Type> W_block00NonConst =
          rcp_const_cast<ThyraOp_Type>(W_block00);
      RCP<TpetraOp_Type> W_tpetra =
          tpetra_extract::getTpetraOperator(W_block00NonConst);

      RCP<TpetraMatrix_Type> W_tpetraMat =
          Teuchos::rcp_dynamic_cast<TpetraMatrix_Type>(W_tpetra);

      XpetraMatrixConstPtr_Type W_matrixXpetra =
          this->getSystem()->getBlock(0, 0)->getXpetraMatrix();
      XpetraMatrixPtr_Type W_matrixXpetraNonConst =
          rcp_const_cast<XpetraMatrix_Type>(W_matrixXpetra);
      Xpetra::CrsMatrixWrap<SC, LO, GO, NO> &crsOp =
          dynamic_cast<Xpetra::CrsMatrixWrap<SC, LO, GO, NO> &>(
              *W_matrixXpetraNonConst);
      Xpetra::TpetraCrsMatrix<SC, LO, GO, NO> &xTpetraMat =
          dynamic_cast<Xpetra::TpetraCrsMatrix<SC, LO, GO, NO> &>(
              *crsOp.getCrsMatrix());
      RCP<TpetraMatrix_Type> tpetraMatXpetra =
          xTpetraMat.getTpetra_CrsMatrixNonConst();
    ...
    if (fill_W_prec) {
      ...
      ParameterListPtr_Type tmpSubList = sublist(
          sublist(sublist(sublist(this->parameterList_, "Teko Parameters"),
                          "Preconditioner Types"),
                  "Teko"),
          "Inverse Factory Library");

      std::string levelCombination1 = tmpSubList->sublist("FROSch-Velocity")
                                          .get("Level Combination", "Additive");
      std::string levelCombination2 = tmpSubList->sublist("FROSch-Pressure")
                                          .get("Level Combination", "Additive");

      if (!levelCombination1.compare("Multiplicative") ||
          !levelCombination2.compare("Multiplicative")) {
        ...
        //                    pListThyraSolver->sublist("Preconditioner
        //                    Types").sublist("FROSch").set("Only apply
        //                    coarse",true);
        //
        //                    Teuchos::RCP<const Thyra::LinearOpBase<SC> >
        //                    thyra_linOp =
        //                    this->getPreconditioner()->getThyraPrec()->getUnspecifiedPrecOp();
        //                    Thyra::apply( *thyra_linOp, Thyra::NOTRANS,
        //                    *thyraB, thyraX.ptr() );
        //                    pListThyraSolver->sublist("Preconditioner
        //                    Types").sublist("FROSch").set("Only apply
        //                    coarse",false);
...
template <class SC, class LO, class GO, class NO>
void NavierStokes<SC, LO, GO, NO>::calculateNonLinResidualVec(
  ...
  // This is ok for bdf with 1.0 scaling of the system. Would be wrong for
  // Crank-Nicolson - might be ok now for CN
...
template <class SC, class LO, class GO, class NO>
void NavierStokes<SC, LO, GO, NO>::calculateNonLinResidualVecWithMeshVelo(
    std::string type, double time, MultiVectorPtr_Type u_minus_w,
    MatrixPtr_Type P) const {
...
NavierStokes<SC, LO, GO, NO>::create_W_op() const {
  this->reAssemble("FixedPoint");
  this->reAssemble("Newton");
...
template <class SC, class LO, class GO, class NO>
Teuchos::RCP<Thyra::LinearOpBase<SC>>
NavierStokes<SC, LO, GO, NO>::create_W_op_Monolithic() const {
...
template <class SC, class LO, class GO, class NO>
Teuchos::RCP<Thyra::LinearOpBase<SC>>
NavierStokes<SC, LO, GO, NO>::create_W_op_Block() const {
  ...
  if (!this->system_->blockExists(1, 1)) {
    MatrixPtr_Type dummy = Teuchos::rcp(
  ...
  Teuchos::RCP<const Thyra::LinearOpBase<SC>> W_opConst =
      Thyra::block2x2(thyraF, thyraBT, thyraB, thyraC);
  Teuchos::RCP<Thyra::LinearOpBase<SC>> W_op =
      Teuchos::rcp_const_cast<Thyra::LinearOpBase<SC>>(W_opConst);
  return W_op;
}
...
NavierStokes<SC, LO, GO, NO>::create_W_prec() const {
