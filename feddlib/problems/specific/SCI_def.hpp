...
// We update the time in multiple component. Also the elements need to be updated with respect to time
// since internal processes like 'active response' are triggered after certain time
template<class SC,class LO,class GO,class NO>
void SCI<SC,LO,GO,NO>::updateTime() const
{
    timeSteppingTool_->t_ = timeSteppingTool_->t_ + timeSteppingTool_->dt_prev_;

   // cout << " ###### Timestep in SCI dt_prev" << timeSteppingTool_->dt_prev_ << " dt= " << timeSteppingTool_->dt_ <<" time= " << timeSteppingTool_->t_ << " ####### " << std::endl;
...
// This is needed for explicit coupling
template<class SC,class LO,class GO,class NO>
    ...
    if(exportHistory)
    {
        ...
        for(int gp =0; gp<4; gp++){
            for(int k=0; k < historyNames.size(); k++){
                //cout << " Export value " << k << " history name " << historyNames[k] << " of gausspoint " << gp << " checkpointtupel " << j << std::endl; 
